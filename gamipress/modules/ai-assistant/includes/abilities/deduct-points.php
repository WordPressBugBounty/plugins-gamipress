<?php
/**
 * Deduct Points Ability
 *
 * @package     GamiPress\AI_Assistant\Ability\Deduct_Points
...
class GamiPress_AI_Assistant_Ability_Deduct_Points extends GamiPress_AI_Assistant_Ability {

    public $ability = 'deduct-points';
    ...
    public function register() {
        ...
        $this->args = array(
            'label'               => __( 'Deduct points in GamiPress', 'gamipress' ),
            'description'         => __( 'Deducts a specified number of GamiPress points to a user. Requires the user, the points type, and the number of points to award.', 'gamipress' ),
            'input_schema'        => array(
                'type'       => 'object',
                'properties' => array(
                    'user' => array(
                    ...
                    'points' => array(
                        'description' => __( 'The number of points to deduct. Must be a positive integer.', 'gamipress' ),
                        'type'        => 'integer',
                    ),
                    'user_earning' => array(
    ...
    public function execute( $args ) {
        ...
        gamipress_deduct_points_to_user( $user->ID, $points, $points_type );
        ...
        if( $user_earning ) {
            ...
            $message = sprintf( __( 'I deducted %1$s to %2$s and registered the earning with the reason "%3$s".', 'gamipress' ), $points_formatted, $user_display, $user_earning_title );
        } else {
            // translators: %1$s: Points amount & label %2$s: User
            $message = sprintf( __( 'I deducted %1$s to %2$s without registering in user earnings.', 'gamipress' ), $points_formatted, $user_display );
        }
...
new GamiPress_AI_Assistant_Ability_Deduct_Points();